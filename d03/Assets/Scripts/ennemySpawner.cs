...
//	public Type type;
	...
	void trySpawn() {
		if (Time.time > nextSpawn && spawned <  waveLenght) {
			...
			botScript.hp = Mathf.RoundToInt(((float)gameManager.instance.nextWaveEnnemyHpUp * (waveNumber - 1) + 100) / 100 * (float)botScript.hp);
			botScript.value = Mathf.RoundToInt(((float)gameManager.instance.nextWaveEnnemyValueUp * (waveNumber - 1) + 100) / 100 * (float)botScript.value);
			nextSpawn = Time.time + spawnRate;
			spawned += 1;
		}
		if (spawned ==  waveLenght) {
			if (waveNumber == gameManager.instance.totalWavesNumber) {
				gameManager.instance.lastWave = true;
				isEmpty = true;
			}
			else {
				nextWave = Time.time + gameManager.instance.delayBetweenWaves;
				pickType();
			}
		}
	}
	...
	void pickType() {
		...
		waveLenght = Mathf.RoundToInt(((float)botScript.waveLenghtModifier + 100) / 100 * (float)gameManager.instance.averageWavesLenght);
		spawnRate = botScript.spawnRate;
	}

	void Update() {
		if (Time.time > nextWave && waveNumber <= gameManager.instance.totalWavesNumber)
			trySpawn ();
	}
