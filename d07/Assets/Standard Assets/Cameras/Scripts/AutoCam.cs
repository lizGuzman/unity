...
namespace UnityStandardAssets.Cameras
{
    ...
    {
		[SerializeField] private Texture2D viseur; // Viseur Texture
		...
		void OnGUI() {
			RaycastHit hit;
			if (Physics.Raycast (Camera.main.ScreenPointToRay (new Vector3 (Screen.width / 2, Screen.height / 2, 0)),out hit, 1000.0f)) {
				//Debug.Log("raycast hit -->" + hit.collider.gameObject.tag);
				if (hit.collider.gameObject.tag == "Tank" && hit.transform.root != me)
					GUI.color = Color.red;
				else
				...
				GUI.color = Color.white;
			GUI.DrawTexture (new Rect (w/2 - 25, h/2 -25, 50, 50), viseur);
		}
	}
}