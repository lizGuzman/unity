...
//		Rear Right Wheel
	...
	IEnumerator FireEffect(int par, Vector3 pos) {
		GameObject show = Instantiate (fireWork[par], pos, Quaternion.identity) as GameObject;
	...
	protected void Fire (Weapon gun, Ray center) {
		if (Shoot [(int)gun]) {
			...
			if (Physics.Raycast (center, out hit)) {
				CarControl car = hit.transform.root.GetComponentInChildren<CarControl> ();
				if (car != null) {
					StartCoroutine(FireEffect((int)gun, hit.transform.position));
					car.takeDamage (gun);
				}
			}
